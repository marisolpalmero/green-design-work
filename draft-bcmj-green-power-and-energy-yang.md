...
In terms of aggregation, Power and Energy consumption sum should
prevent double-counting. The sum depends on the energy object, when it
comes to the PoE port as energy object, let's say that the port consumes
1 watts, and provides 9 watts to an IP phone, attached via PoE, what
would operators prefer the energy object to report at the device level?
do operators sum the total-energy-consumed or total-energy-produced?
in another word, total-energy-consumed = 1 or 10 Watts? and what should
the energy object reports at the network level? In case the PoE phone
can also reports its 9 watts, then at the network level, double-counting
must be avoided.

Operators might not always be interested to get the individual component
accuracy. What counts is the device level or domain level, to meet their
demands, identity accuracy-like-parent is introduced as follows,
...
interval issues, operators should use as much as possible this
accuracy-like-parent identity.
